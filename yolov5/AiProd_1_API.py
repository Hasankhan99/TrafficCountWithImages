    ...
    def get_result():
        while True:
            try: 
                response = requests.post(api_token, headers=headers)
                if response.status_code == 200:
                    CheckPreData(suid,date,'/home/live/Documents/api_programst/db.sqlite3')
                    return json.loads(response.content.decode('utf-8'))
...
def logic():
    ...
    with conn:
        mainSetting = getMainSetting(conn, 1) 
        # Check_dir(mainSetting[5])
        stores = selectAllStores(conn, globalDate, 'Aiprod1')
        if not mainSetting['is_system_date']:
            globalDate = mainSetting['date_for_all_store']

        for i, storeDataObj in enumerate(stores):
            # if not i<3:
            #     continue
            ab+=1
